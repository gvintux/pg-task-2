...
def deflection_solve(**specs):
    ...
    print('W1')
    pprint(
        w1_coeff.factor(deep=True).collect(I).collect(D).collect(tf).collect(K_sym).collect(tf).collect(lm ** 2 * v ** 2))
    print('W2')
    pprint(w2_coeff.factor(deep=True).collect(I).collect(D))
    pprint(
        w_le_den.factor(deep=True).collect(I).collect(D).collect(tf).collect(K_sym).collect(tf).collect(lm ** 2 * v ** 2))
    exit(0)
