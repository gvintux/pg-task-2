...
def deflection_solve(**specs):
    ...
    dw_y = diff(w_num, y, 2)
    d2w_y = diff(dw_y, y, 2)
    d2w_xy = diff(dw_x, x, y)
    d2w_x_t = diff(dw_t, x, 2)
    d2w_y_t = diff(dw_t, t, 2)
    d2w_xy_t = diff(dw_t, x, y)
    print('dw_t:')
    pprint(dw_t.collect(delta).collect([k1_sym,k2_sym]))
    print('d2w_x:')
    pprint(d2w_x.collect(delta).collect([k1_sym,k2_sym]))
    print('d2w_y:')
    pprint(d2w_y.collect(delta).collect([k1_sym,k2_sym]))
    print('d2w_xy:')
    pprint(d2w_xy.collect(delta).collect([k1_sym,k2_sym]))
    print('d2w_x_t:')
    pprint(d2w_x_t.collect(delta).collect([k1_sym,k2_sym]))
    print('d2w_y_t:')
